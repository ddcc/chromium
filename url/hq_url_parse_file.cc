...
void DoParseUNC(const CHAR* spec,
  ...
  if (next_slash == spec_len) {
    ...
    if (host_len)
      parsed->host = HQ_Component(after_slashes, host_len);
    else
      parsed->host.reset();
    parsed->path.reset();
    return;
  ...
  if (host_len)
    parsed->host = MakeHQRange(after_slashes, next_slash);
  else
    parsed->host.reset();
  if (next_slash < spec_len) {
    ParsePathInternal(spec, MakeRange(next_slash, spec_len), &parsed->path,
...
template <typename CHAR>
void DoParseFileURL(const CHAR* spec, int spec_len, HQ_Parsed* parsed) {
  DCHECK(spec_len >= 0);
