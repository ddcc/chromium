...
HQ_GURL::HQ_GURL() : is_valid_(false) {}

HQ_GURL::HQ_GURL(const GURL& other)
    : spec_(other.possibly_invalid_spec()),
      is_valid_(other.is_valid()),
      parsed_(other.parsed_for_possibly_invalid_spec()) {
  if (other.inner_url())
    inner_url_.reset(new HQ_GURL(*other.inner_url()));
  // Valid filesystem urls should always have an inner_url_.
  DCHECK(!is_valid_ || !SchemeIsFileSystem() || inner_url_);
}

HQ_GURL::HQ_GURL(const HQ_GURL& other)
    : spec_(other.spec_), is_valid_(other.is_valid_), parsed_(other.parsed_) {
  if (other.inner_url_)
    inner_url_.reset(new HQ_GURL(*other.inner_url_));
  // Valid filesystem urls should always have an inner_url_.
  DCHECK(!is_valid_ || !SchemeIsFileSystem() || inner_url_);
}

HQ_GURL::HQ_GURL(HQ_GURL&& other) noexcept
    : spec_(std::move(other.spec_)),
      is_valid_(other.is_valid_),
      parsed_(other.parsed_),
      inner_url_(std::move(other.inner_url_)) {
  other.is_valid_ = false;
  other.parsed_ = url::HQ_Parsed();
}

HQ_GURL::HQ_GURL(base::StringPiece url_string) {
  InitCanonical(url_string, true);
}

HQ_GURL::HQ_GURL(base::StringPiece16 url_string) {
  InitCanonical(url_string, true);
}

HQ_GURL::HQ_GURL(const std::string& url_string, RetainWhiteSpaceSelector) {
  InitCanonical(base::StringPiece(url_string), false);
}

HQ_GURL::HQ_GURL(const char* canonical_spec,
...
HQ_GURL::HQ_GURL(std::string canonical_spec,
                 const url::HQ_Parsed& parsed,
                 bool is_valid)
...
void HQ_GURL::InitCanonical(base::BasicStringPiece<STR> input_spec,
                            bool trim_path_end) {
  std::string spec;
  url::StdStringCanonOutput output(&spec);
  is_valid_ = url::Canonicalize(input_spec.data(),
                                static_cast<int>(input_spec.length()),
                                trim_path_end, NULL, &output, &parsed_);
  output.Complete();  // Must be done before using string.
  spec_ = std::move(spec);
  if (is_valid_ && SchemeIsFileSystem()) {
    inner_url_.reset(new HQ_GURL(spec_.data(), parsed_.Length(),
...
void HQ_GURL::InitializeFromCanonicalSpec() {
  if (is_valid_ && SchemeIsFileSystem()) {
    inner_url_.reset(new HQ_GURL(spec_.data(), parsed_.Length(),
                                 *parsed_.inner_parsed(), true));
  }
  ...
  if (is_valid_) {
    ...
    if (!url::FindAndCompareScheme(spec_.data(), spec_.length(),
      ...
      HQ_GURL test_url(spec_, RETAIN_TRAILING_PATH_WHITEPACE);
...
HQ_GURL::~HQ_GURL() = default;

HQ_GURL& HQ_GURL::operator=(const GURL& other) {
  spec_ = other.possibly_invalid_spec();
  is_valid_ = other.is_valid();
  parsed_ = other.parsed_for_possibly_invalid_spec();
...
HQ_GURL& HQ_GURL::operator=(const HQ_GURL& other) {
  spec_ = other.spec_;
  is_valid_ = other.is_valid_;
  parsed_ = other.parsed_;
...
HQ_GURL& HQ_GURL::operator=(HQ_GURL&& other) noexcept {
  spec_ = std::move(other.spec_);
  is_valid_ = other.is_valid_;
  parsed_ = other.parsed_;
  inner_url_ = std::move(other.inner_url_);
...
const std::hq_string& HQ_GURL::spec() const {
  if (is_valid_ || spec_.empty())
    return spec_;

  DCHECK(false) << "Trying to get the spec of an invalid URL!";
  return base::EmptyHQString();
}

bool HQ_GURL::operator<(const HQ_GURL& other) const {
...
HQ_GURL HQ_GURL::Resolve(base::StringPiece relative) const {
  ...
  if (!url::ResolveRelative(spec_.data(), static_cast<int>(spec_.length()),
                            ...
                            static_cast<int>(relative.length()), nullptr,
  ...
  if (result.SchemeIsFileSystem()) {
    result.inner_url_.reset(new HQ_GURL(result.spec_.data(),
...
HQ_GURL HQ_GURL::Resolve(base::StringPiece16 relative) const {
  // Not allowed for invalid URLs.
  if (!is_valid_)
    return HQ_GURL();
...
// Note: code duplicated below (it's inconvenient to use a template here).
HQ_GURL HQ_GURL::ReplaceComponents(
...
HQ_GURL HQ_GURL::GetOrigin() const {
...
HQ_GURL HQ_GURL::GetAsReferrer() const {
...
HQ_GURL HQ_GURL::GetWithEmptyPath() const {
  // This doesn't make sense for invalid or nonstandard URLs, so return
  // the empty URL.
  if (!is_valid_ || !IsStandard())
    return HQ_GURL();
...
HQ_GURL HQ_GURL::GetWithoutFilename() const {
  return Resolve(".");
}

bool HQ_GURL::IsStandard() const {
  return url::IsStandard(spec_.data(), parsed_.scheme);
}

bool HQ_GURL::IsAboutBlank() const {
  return IsAboutUrl(url::kAboutBlankPath);
}

bool HQ_GURL::IsAboutSrcdoc() const {
  return IsAboutUrl(url::kAboutSrcdocPath);
}

bool HQ_GURL::SchemeIs(base::StringPiece lower_ascii_scheme) const {
...
bool HQ_GURL::SchemeIsHTTPOrHTTPS() const {
  return SchemeIs(url::kHttpScheme) || SchemeIs(url::kHttpsScheme);
}

bool HQ_GURL::SchemeIsWSOrWSS() const {
  return SchemeIs(url::kWsScheme) || SchemeIs(url::kWssScheme);
}

bool HQ_GURL::SchemeIsCryptographic() const {
  if (parsed_.scheme.len <= 0)
    return false;
  return SchemeIsCryptographic(scheme_piece());
}

bool HQ_GURL::SchemeIsCryptographic(base::StringPiece lower_ascii_scheme) {
  DCHECK(base::IsStringASCII(lower_ascii_scheme));
  DCHECK(base::ToLowerASCII(lower_ascii_scheme) == lower_ascii_scheme);
...
std::string HQ_GURL::ExtractFileName() const {
...
base::StringPiece HQ_GURL::PathForRequestPiece() const {
...
std::string HQ_GURL::PathForRequest() const {
  return PathForRequestPiece().as_string();
}

std::string HQ_GURL::HostNoBrackets() const {
  return HostNoBracketsPiece().as_string();
}

base::StringPiece HQ_GURL::HostNoBracketsPiece() const {
...
std::string HQ_GURL::GetContent() const {
  if (!is_valid_)
    return std::string();
...
bool HQ_GURL::HostIsIPAddress() const {
  return is_valid_ && url::HostIsIPAddress(host_piece());
}

const HQ_GURL& HQ_GURL::EmptyHQ_GURL() {
  static base::NoDestructor<HQ_GURL> empty_HQ_GURL;
  return *empty_HQ_GURL;
}

bool HQ_GURL::DomainIs(base::StringPiece canonical_domain) const {
  if (!is_valid_)
    return false;
...
bool HQ_GURL::EqualsIgnoringRef(const HQ_GURL& other) const {
...
void HQ_GURL::Swap(HQ_GURL* other) {
...
size_t HQ_GURL::EstimateMemoryUsage() const {
...
bool HQ_GURL::IsAboutUrl(base::StringPiece allowed_path) const {
...
bool operator==(const HQ_GURL& x, const HQ_GURL& y) {
  return x.possibly_invalid_spec() == y.possibly_invalid_spec();
}

bool operator==(const HQ_GURL& x, const GURL& y) {
  return x.possibly_invalid_spec() == y.possibly_invalid_spec();
}

bool operator==(const GURL& x, const HQ_GURL& y) {
  return x.possibly_invalid_spec() == y.possibly_invalid_spec();
}
...
bool operator==(const HQ_GURL& x, const base::StringPiece& spec) {
  DCHECK_EQ(HQ_GURL(spec).possibly_invalid_spec().str(), spec)
      << "Comparisons of HQ_GURLs and strings must ensure as a precondition "
         "that "
         "the string is fully canonicalized.";
  return x.possibly_invalid_spec().str() == spec;
}
