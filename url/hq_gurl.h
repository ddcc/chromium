...
#ifndef URL_HQ_GURL_H_
#define URL_HQ_GURL_H_
...
// Represents a URL. HQ_GURL is Google's URL parsing library.
//
...
class COMPONENT_EXPORT(URL) HQ_GURL {
 public:
  ...
  HQ_GURL(const GURL& other);
  HQ_GURL(const HQ_GURL& other);
  HQ_GURL(HQ_GURL&& other) noexcept;

  // The strings to this contructor should be UTF-8 / UTF-16.
  explicit HQ_GURL(base::StringPiece url_string);
  explicit HQ_GURL(base::StringPiece16 url_string);

  // Constructor for URLs that have already been parsed and canonicalized. This
  // is used for conversions from KURL, for example. The caller must supply all
  // information associated with the URL, which must be correct and consistent.
  HQ_GURL(const char* canonical_spec,
          size_t canonical_spec_len,
          const url::HQ_Parsed& parsed,
          bool is_valid);
  ...
  // implementation, we use std::move to move the data into the HQ_GURL itself,
  // which means we end up with zero copies.
  HQ_GURL(std::string canonical_spec,
          const url::HQ_Parsed& parsed,
          bool is_valid);

  ~HQ_GURL();

  HQ_GURL& operator=(const GURL& other);
  HQ_GURL& operator=(const HQ_GURL& other);
  HQ_GURL& operator=(HQ_GURL&& other) noexcept;

  GURL gurl() const {
    return GURL(spec_.str(), parsed_.parsed(), is_valid_);
  }
  ...
  const std::hq_string& spec() const;
  ...
  const std::hq_string& possibly_invalid_spec() const { return spec_; }
  ...
  const url::HQ_Parsed& parsed_for_possibly_invalid_spec() const {
    return parsed_;
  }

  // Allows HQ_GURL to used as a key in STL (for example, a std::set or
  // std::map).
  bool operator<(const HQ_GURL& other) const;
  bool operator>(const HQ_GURL& other) const;
  ...
  // resolve it. In these cases, the result will be an empty, invalid HQ_GURL.
  //
  ...
  HQ_GURL Resolve(base::StringPiece relative) const;
  HQ_GURL Resolve(base::StringPiece16 relative) const;

  // Creates a new HQ_GURL by replacing the current URL's components with the
  ...
  HQ_GURL ReplaceComponents(const url::Replacements<char>& replacements) const;
  HQ_GURL ReplaceComponents(
  ...
  // an empty, invalid HQ_GURL. Note that this *does* work for file: URLs, which
  // some callers may want to filter out before calling this.
  //
  // It is an error to get an empty path on an invalid URL. The result
  // will be the empty URL.
  HQ_GURL GetWithEmptyPath() const;

  // A helper function to return a HQ_GURL without the filename, query values,
  // and fragment. For example,
  // HQ_GURL("https://www.foo.com/index.html?q=test").GetWithoutFilename().spec()
  // will return "https://www.foo.com/".
  // HQ_GURL("https://www.foo.com/bar/").GetWithoutFilename().spec()
  // will return "https://www.foo.com/bar/". If the HQ_GURL is invalid or
  // missing a scheme, authority or path, it will return an empty, invalid
  // HQ_GURL.
  HQ_GURL GetWithoutFilename() const;

  // A helper function to return a HQ_GURL containing just the scheme, host,
  ...
  // invalid HQ_GURL. If the URL has neither username nor password, this
  // degenerates to GetWithEmptyPath().
  //
  // It is an error to get the origin of an invalid URL. The result
  // will be the empty URL.
  HQ_GURL GetOrigin() const;

  // A helper function to return a HQ_GURL stripped from the elements that are
  // not supposed to be sent as HTTP referrer: username, password and ref
  // fragment. For invalid URLs or URLs that no valid referrers, an empty URL
  // will be returned.
  HQ_GURL GetAsReferrer() const;
  ...
  bool SchemeIsFileSystem() const { return SchemeIs(url::kFileSystemScheme); }
  ...
  // values defined in HQ_Parsed for ExtractPort.
  int IntPort() const;
  ...
  // the # character).
  bool EqualsIgnoringRef(const HQ_GURL& other) const;

  // Swaps the contents of this HQ_GURL object with |other|, without doing
  // any memory allocations.
  void Swap(HQ_GURL* other);

  // Returns a reference to a singleton empty HQ_GURL. This object is for
  // callers who return references but don't have anything to return in some
  // cases. If you just want an empty URL for normal use, prefer HQ_GURL(). This
  // function may be called from any thread.
  static const HQ_GURL& EmptyHQ_GURL();
  ...
  // caling spec() on the HQ_GURL itself. This should be fixed.
  // See https://crbug.com/619596
  const HQ_GURL* inner_url() const { return inner_url_.get(); }
 ...
 private:
  ...
  HQ_GURL(const std::string& url_string, RetainWhiteSpaceSelector);
  ...
  std::string ComponentString(const url::HQ_Component& comp) const {
    if (comp.len <= 0)
      return std::string();
    return std::string(spec_, comp.begin, comp.len);
  }
  ...
  base::StringPiece ComponentStringPiece(const url::HQ_Component& comp) const {
    if (comp.len <= 0)
      return base::StringPiece();
    return base::StringPiece(&spec_[comp.begin], comp.len);
  }
  ...
  std::hq_wrapper<bool> is_valid_;

  // Identified components of the canonical spec.
  url::HQ_Parsed parsed_;

  // Used for nested schemes [currently only filesystem:].
  std::hq_unique_ptr<HQ_GURL> inner_url_;
};

// Stream operator so HQ_GURL can be used in assertion statements.
COMPONENT_EXPORT(URL)
std::ostream& operator<<(std::ostream& out, const HQ_GURL& url);

COMPONENT_EXPORT(URL) bool operator==(const HQ_GURL& x, const HQ_GURL& y);
COMPONENT_EXPORT(URL) bool operator==(const HQ_GURL& x, const GURL& y);
COMPONENT_EXPORT(URL) bool operator==(const GURL& x, const HQ_GURL& y);
COMPONENT_EXPORT(URL) bool operator!=(const HQ_GURL& x, const HQ_GURL& y);
COMPONENT_EXPORT(URL) bool operator!=(const HQ_GURL& x, const GURL& y);
COMPONENT_EXPORT(URL) bool operator!=(const GURL& x, const HQ_GURL& y);

// Equality operator for comparing raw spec_. This should be used in place of
// url == HQ_GURL(spec) where |spec| is known (i.e. constants). This is to
// prevent needlessly re-parsing |spec| into a temporary HQ_GURL.
COMPONENT_EXPORT(URL)
bool operator==(const HQ_GURL& x, const base::StringPiece& spec);
COMPONENT_EXPORT(URL)
bool operator==(const base::StringPiece& spec, const HQ_GURL& x);
COMPONENT_EXPORT(URL)
bool operator!=(const HQ_GURL& x, const base::StringPiece& spec);
COMPONENT_EXPORT(URL)
bool operator!=(const base::StringPiece& spec, const HQ_GURL& x);

// DEBUG_ALIAS_FOR_HQ_GURL(var_name, url) copies |url| into a new
// stack-allocated variable named |<var_name>|.  This helps ensure that the
// value of |url| gets preserved in crash dumps.
#define DEBUG_ALIAS_FOR_HQ_GURL(var_name, url) \
  DEBUG_ALIAS_FOR_CSTR(var_name, (url).possibly_invalid_spec().c_str(), 128)

#endif  // URL_HQ_GURL_H_
