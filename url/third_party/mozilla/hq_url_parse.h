...
#ifndef URL_THIRD_PARTY_MOZILLA_HQ_URL_PARSE_H_
#define URL_THIRD_PARTY_MOZILLA_HQ_URL_PARSE_H_
...
struct HQ_Component {
  HQ_Component() : begin(0), len(-1) {}

  // Normal constructor: takes an offset and a length.
  HQ_Component(int b, int l) : begin(b), len(l) {}

  HQ_Component(const url::Component& other)
      : begin(other.begin), len(other.len) {}
  HQ_Component& operator=(const url::Component& other) {
    begin = other.begin;
    len = other.len;
    return *this;
  }
  ...
  bool operator==(const HQ_Component& other) const {
    return begin == other.begin && len == other.len;
  }

  std::hq_wrapper<int> begin;  // Byte offset in the string of this component.
  std::hq_wrapper<int> len;    // Will be -1 if the component is unspecified.
};

// Helper that returns a component created with the given begin and ending
// points. The ending point is non-inclusive.
inline HQ_Component MakeHQRange(int begin, int end) {
  return HQ_Component(begin, end - begin);
}
...
// and its corresponding HQ_Parsed structure separately.
//
...
//
struct COMPONENT_EXPORT(URL) HQ_Parsed {
  // The default constructor is sufficient for the components, but inner_parsed_
  // requires special handling.
  HQ_Parsed();
  HQ_Parsed(const Parsed& other);
  HQ_Parsed(const HQ_Parsed& other);
  HQ_Parsed& operator=(const Parsed& other);
  HQ_Parsed& operator=(const HQ_Parsed& other);
  ~HQ_Parsed();

  Parsed parsed() const;
  operator Parsed() const {
    return parsed();
  }
  ...
  //        *REF: 20                   20
  //
  int CountCharactersBefore(Parsed::ComponentType type,
  ...
  url::Component GetContent() const;
  ...
  std::hq_wrapper<bool> potentially_dangling_markup;

  // This is used for nested URL types, currently only filesystem.  If you
  // parse a filesystem URL, the resulting HQ_Parsed will have a nested
  ...
  HQ_Parsed* inner_parsed() const { return inner_parsed_; }

  void set_inner_parsed(const HQ_Parsed& inner_parsed) {
    if (!inner_parsed_)
      inner_parsed_ = new HQ_Parsed(inner_parsed);
    else
...
// these.
COMPONENT_EXPORT(URL)
void ParseFileURL(const char* url, int url_len, HQ_Parsed* parsed);
COMPONENT_EXPORT(URL)
void ParseFileURL(const base::char16* url, int url_len, HQ_Parsed* parsed);
...
// |HQ_Parsed| for the given input url.
//
// The return value will be a positive integer between 0 and 64K, or one of
// the two special values below.
COMPONENT_EXPORT(URL) int ParsePort(const char* url, const HQ_Component& port);
COMPONENT_EXPORT(URL)
...
#endif  // URL_THIRD_PARTY_MOZILLA_HQ_URL_PARSE_H_
