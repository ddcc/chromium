...
void ParseUserInfo(const CHAR* spec,
                   const HQ_Component& user,
                   HQ_Component* username,
                   HQ_Component* password) {
  ...
  if (colon_offset < user.len) {
    ...
    *username = HQ_Component(user.begin, colon_offset);
    *password =
        MakeHQRange(user.begin + colon_offset + 1, user.begin + user.len);
  } else {
    // No separator, treat everything as the username
    *username = user;
    *password = HQ_Component();
  }
}

template <typename CHAR>
void ParseServerInfo(const CHAR* spec,
                     const HQ_Component& serverinfo,
                     HQ_Component* hostname,
                     HQ_Component* port_num) {
  if (serverinfo.len == 0) {
  ...
  if (colon > ipv6_terminator) {
    ...
    *hostname = MakeHQRange(serverinfo.begin, colon);
    if (hostname->len == 0)
      hostname->reset();
    *port_num = MakeHQRange(colon + 1, serverinfo.end());
  } else {
    // No port: <hostname>
    *hostname = serverinfo;
    port_num->reset();
...
void DoParseAuthority(const CHAR* spec,
  ...
  if (spec[i] == '@') {
    ...
    ParseUserInfo(spec, HQ_Component(auth.begin, i - auth.begin), username,
                  password);
    ParseServerInfo(spec, MakeHQRange(i + 1, auth.begin + auth.len), hostname,
                    port_num);
  } else {
...
void ParsePath(const CHAR* spec,
  ...
  if (ref_separator >= 0) {
    ...
    *ref = MakeHQRange(ref_separator + 1, path_end);
  } else {
  ...
  if (query_separator >= 0) {
    ...
    *query = MakeHQRange(query_separator + 1, query_end);
  } else {
    query->reset();
  }
  ...
  if (file_end != path.begin)
    *filepath = MakeHQRange(path.begin, file_end);
  else
    filepath->reset();
}

template <typename CHAR>
bool DoExtractScheme(const CHAR* url, int url_len, HQ_Component* scheme) {
...
// Fills in all members of the HQ_Parsed structure except for the scheme.
//
...
void DoParseAfterScheme(const CHAR* spec,
  ...
  url::Component authority;
  url::Component full_path;
  ...
  authority = url::Component(after_slashes, end_auth - after_slashes);

  if (end_auth == spec_len)  // No beginning of path found.
    full_path = url::Component();
  else  // Everything starting from the slash to the end is the path.
    full_path = url::Component(end_auth, spec_len - end_auth);
...
// host, path, etc.
template <typename CHAR>
void DoParseStandardURL(const CHAR* spec, int spec_len, HQ_Parsed* parsed) {
  DCHECK(spec_len >= 0);
...
template <typename CHAR>
void DoParseFileSystemURL(const CHAR* spec, int spec_len, HQ_Parsed* parsed) {
  DCHECK(spec_len >= 0);
...
void DoParsePathURL(const CHAR* spec,
  ...
  ParsePath(spec, MakeHQRange(path_begin, spec_len), &parsed->path,
            &parsed->query, &parsed->ref);
}

template <typename CHAR>
void DoParseMailtoURL(const CHAR* spec, int spec_len, HQ_Parsed* parsed) {
  DCHECK(spec_len >= 0);
  ...
  for (int i = path_begin; i < path_end; ++i) {
    if (spec[i] == '?') {
      parsed->query = MakeHQRange(i + 1, path_end);
      path_end = i;
      break;
    }
  }
  ...
  if (path_begin == path_end) {
    ...
    parsed->path = MakeHQRange(path_begin, path_end);
  }
}
...
template <typename CHAR>
int DoParsePort(const CHAR* spec, const HQ_Component& component) {
  ...
  url::Component digits_comp(component.end(), 0);
...
void DoExtractFileName(const CHAR* spec,
  ...
  for (int i = path.end() - 1; i >= path.begin; i--) {
    if (spec[i] == ';') {
      ...
      *file_name = MakeHQRange(i + 1, file_end);
      return;
    }
  }
  ...
  *file_name = MakeHQRange(path.begin, file_end);
  return;
}
...
HQ_Parsed::HQ_Parsed()
    : potentially_dangling_markup(false), inner_parsed_(NULL) {}

HQ_Parsed::HQ_Parsed(const HQ_Parsed& other)
    : scheme(other.scheme),
...
HQ_Parsed::HQ_Parsed(const Parsed& other)
    : scheme(other.scheme),
      username(other.username),
      password(other.password),
      host(other.host),
...
HQ_Parsed& HQ_Parsed::operator=(const Parsed& other) {
  scheme = other.scheme;
  username = other.username;
  password = other.password;
  host = other.host;
...
HQ_Parsed& HQ_Parsed::operator=(const HQ_Parsed& other) {
  if (this != &other) {
    scheme = other.scheme;
    username = other.username;
    password = other.password;
...
Parsed HQ_Parsed::parsed() const {
  Parsed p;
  p.scheme = scheme.component();
  p.username = username.component();
  p.password = password.component();
  p.host = host.component();
  p.port = port.component();
  p.path = path.component();
  p.query = query.component();
  p.ref = ref.component();
  p.potentially_dangling_markup = potentially_dangling_markup;
  if (inner_parsed_)
    p.set_inner_parsed(inner_parsed_->parsed());
  return p;
}

int HQ_Parsed::Length() const {
  if (ref.is_valid())
    return ref.end();
  return CountCharactersBefore(Parsed::REF, false);
}

int HQ_Parsed::CountCharactersBefore(Parsed::ComponentType type,
                                     bool include_delimiter) const {
  if (type == Parsed::SCHEME)
    return scheme.begin;
  ...
  if (port.is_valid()) {
    if (type < Parsed::PORT || (type == Parsed::PORT && include_delimiter))
  ...
  if (query.is_valid()) {
    if (type < Parsed::QUERY || (type == Parsed::QUERY && include_delimiter))
      return query.begin - 1;  // Back over delimiter.
    if (type == Parsed::QUERY)
      return query.begin;  // Don't want delimiter counted.
    cur = query.end();
  }

  if (ref.is_valid()) {
    if (type == Parsed::REF && !include_delimiter)
...
url::Component HQ_Parsed::GetContent() const {
  const int begin = CountCharactersBefore(Parsed::USERNAME, false);
...
bool ExtractScheme(const char* url, int url_len, HQ_Component* scheme) {
  return DoExtractScheme(url, url_len, scheme);
}

bool ExtractScheme(const base::char16* url, int url_len, HQ_Component* scheme) {
  return DoExtractScheme(url, url_len, scheme);
}
...
int ParsePort(const char* url, const HQ_Component& port) {
  return DoParsePort(url, port);
}

int ParsePort(const base::char16* url, const HQ_Component& port) {
  return DoParsePort(url, port);
}

void ParseStandardURL(const char* url, int url_len, HQ_Parsed* parsed) {
  DoParseStandardURL(url, url_len, parsed);
}

void ParseStandardURL(const base::char16* url, int url_len, HQ_Parsed* parsed) {
  DoParseStandardURL(url, url_len, parsed);
}
...
void ParseFileSystemURL(const char* url, int url_len, HQ_Parsed* parsed) {
  DoParseFileSystemURL(url, url_len, parsed);
}

void ParseFileSystemURL(const base::char16* url,
                        int url_len,
                        HQ_Parsed* parsed) {
  DoParseFileSystemURL(url, url_len, parsed);
}

void ParseMailtoURL(const char* url, int url_len, HQ_Parsed* parsed) {
  DoParseMailtoURL(url, url_len, parsed);
}

void ParseMailtoURL(const base::char16* url, int url_len, HQ_Parsed* parsed) {
  DoParseMailtoURL(url, url_len, parsed);
}
