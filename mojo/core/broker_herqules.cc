...
Channel::MessagePtr WaitForBrokerMessage(
  ...
  HerQulesWaitRead(header, read);

  Channel::MessagePtr message = Channel::Message::Deserialize(
      HerQulesGetMessage(header, read), HerQulesGetWrite(header) - read);
  if (!message) {
...
bool WaitForHandshake(int fd,
  ...
  Channel::MessagePtr msg =
      HerQulesParseHandshake(fd, HerQulesGetMessage(header, read),
                             HerQulesGetWrite(header) - read, &handles);
  if (!msg) {
...
bool SendMessage(int fd,
  ...
  bool ret = true;
  const auto size = message->data_num_bytes();
  HerQulesShmHdr* header = wr_mapping.GetMemoryAs<HerQulesShmHdr>();

retry:
  HerQulesLock(header);
  if (!HerQulesMaySend(header, size, wr_mapping.size())) {
    HerQulesWait(header, nullptr);
    HerQulesUnlock(header);
    goto retry;
  }

  if (HerQulesSend(fd, HerQulesGetMessage(header, HerQulesGetWrite(header)),
                   std::move(message))) {
