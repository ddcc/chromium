...
class ChannelHerQules : public Channel,
 ...
 public:
  ...
  bool GetReadPlatformHandles(const void* payload,
    ...
    if (!HerQulesParseMessageHandles(
      ...
      LOG(ERROR) << "Cannot pop transmitted fd!";
      return false;
    }
 ...
 private:
  ...
  void InitReceive() {
    ...
    base::CurrentSHMThread::Get()->WatchMemoryRegion(
        receive_region_, true, base::MessagePumpForSHM::WATCH_READ,
  ...
  void InitSend() {
    ...
    base::CurrentSHMThread::Get()->WatchMemoryRegion(
        send_region_, true, base::MessagePumpForSHM::WATCH_WRITE,
  ...
  // base::MessagePumpForSHM::ShmWatcher:
  size_t OnShmMayWrite() override {
    base::AutoLock lock(write_lock_);
    if (outgoing_messages_.size()) {
      return outgoing_messages_.front()->data_num_bytes();
    }
  ...
  // base::MessagePumpForSHM::ShmWatcher:
  bool OnShmDoWrite(int fd, uint8_t* memory) override {
    base::AutoLock lock(write_lock_);
    MessagePtr message = std::move(outgoing_messages_.front());
    outgoing_messages_.pop_front();

    if (!HerQulesSend(fd, memory, std::move(message))) {
