...
#ifndef CONTENT_BROWSER_SCHEDULER_BROWSER_SHM_THREAD_DELEGATE_H_
#define CONTENT_BROWSER_SCHEDULER_BROWSER_SHM_THREAD_DELEGATE_H_
...
// Delegate for the SHM thread.
class CONTENT_EXPORT BrowserSHMThreadDelegate : public base::Thread::Delegate {
 public:
  using Handle = BrowserTaskQueues::Handle;

  // Creates a BrowserSHMThreadDelegate for use with a real SHM thread.
  BrowserSHMThreadDelegate();
  ~BrowserSHMThreadDelegate() override;

  static std::unique_ptr<BrowserSHMThreadDelegate> CreateForTesting(
      base::sequence_manager::SequenceManager* sequence_manager) {
    DCHECK(sequence_manager);
    return base::WrapUnique(new BrowserSHMThreadDelegate(sequence_manager));
  }
 ...
 private:
  ...
  // Creates a sequence funneled BrowserSHMThreadDelegate for use in testing.
  // Installs TLSMultiplexer which allows ensures the right results for
  // base::CurrentThread when running an "SHM Thread" task.
  explicit BrowserSHMThreadDelegate(
      base::sequence_manager::SequenceManager* sequence_manager);
  ...
  // In unit tests the SHM "thread" can be sequence funneled onto the main
  // thread so we need to multiplex the TLS binding to ensure
  // base::CurrentThread behaves as expected.
...
#endif  // CONTENT_BROWSER_SCHEDULER_BROWSER_SHM_THREAD_DELEGATE_H_
